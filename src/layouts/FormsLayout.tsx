...
function FormsLayout({ children, pageTitle, pageIcon }: FormsLayoutProps) {
  ...
  const navItems = allNavItems.filter(item => {
    if (location.pathname === '/dashboard') {
      return item.label === 'Module Portal';
    }
    if (location.pathname === '/dashboard/forms') {
      return item.label === 'Module Portal' || item.label === 'Forms';
    }
    // Default fallback for other paths that might use FormsLayout
    return true;
  });
